...
def gestorDeUMs(conn):
    ...
    for device in tarjeta:
            threads[device[1]] = threading.Thread(target=UMhandler(macTarjeta,device[0],device[1],port))
            port= port+1
    finOrdenie = False
         
    # cargar modelo
    clf = load_data(filenameCLF)
    for p in threads:
        threads[p].start()
    while True:
        try:
...
def UMhandler(adapter,bd_addr,idDevice,port):
    filenameLog = "logs/"+idDevice+"_"+str(datetime.datetime.now())+".log"
    log(idDevice+"\n",filenameLog)
    def hayPaquete():
    ...
    finVaca = False
    filename = ""
    path = "Datos/"+str(idDevice)+"_"
    while True:
        try:                        
            if conectarUM:
                 sock_blu = BluetoothSocket( RFCOMM )
                 # me conecto
